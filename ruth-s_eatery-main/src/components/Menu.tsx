...
const Menu = () => {
  ...
  const printMenuPdf = () => {
    const printableMenu = data.menu.map(section => `
      <section style="margin-bottom: 28px;">
        <h2 style="margin-bottom: 4px; color: ${data.theme.accentColor};">${section.category}</h2>
        <p style="margin-top: 0; color: #666;">${section.description || ''}</p>
        ${section.items.map(item => `
          <div style="display: flex; justify-content: space-between; gap: 24px; border-bottom: 1px solid #eee; padding: 10px 0;">
            <div>
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Ruthy Eatery Full Menu</title>
          <style>
        ...
        <body>
          <h1>Ruthy Eatery Full Menu</h1>
          <p style="color: #666; margin-bottom: 32px;">Generated from current database records.</p>
          ${printableMenu}
          <button onclick="window.print()" style="margin-top: 24px; padding: 12px 18px; border: 0; border-radius: 999px; background: ${data.theme.accentColor}; color: white; font-weight: bold;">Print / Save as PDF</button>
    ...
    <section id="menu" className="py-24" style={{ backgroundColor: data.theme.bgColor }}>
      <div className="container mx-auto px-6">
        ...
        <div className="mt-20 text-center">
          <button
            onClick={printMenuPdf}
            className="border-2 border-amber-600 text-amber-600 hover:bg-amber-600 hover:text-white px-10 py-3 rounded-full font-bold transition-all"
          >
