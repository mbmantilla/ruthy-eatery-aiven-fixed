  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      if (mode === 'login') {
        ...
        setError('Unable to create account. This email may already be registered. Try logging in instead.');
      }
    }
  };
          ...
          >
            ...
            <div className="p-8 md:p-10">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {error && (
                  <div className="bg-red-50 text-red-600 border border-red-100 px-4 py-3 rounded-2xl text-sm font-semibold">
                    {error}
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full bg-gray-900 text-white font-bold py-5 rounded-[1.5rem] hover:bg-gray-800 transition-all flex items-center justify-center gap-2 shadow-xl shadow-gray-900/10 disabled:opacity-60"
                >
                  <span>{isSubmitting ? 'Please wait...' : mode === 'login' ? 'Sign In' : 'Sign Up'}</span>
