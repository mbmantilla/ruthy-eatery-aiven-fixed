...
interface SiteContextType {
  ...
  addMessage: (message: Omit<SiteMessage, 'id' | 'date' | 'isRead'>) => Promise<void>;
...
const normalizeSiteData = (incoming: Partial<SiteData> | null | undefined): SiteData => ({
  ...defaultData,
  ...(incoming || {}),
  ...
  users: Array.isArray(incoming?.users) && incoming.users.length ? incoming.users : defaultData.users,
});
...
export const SiteProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Fetch from Aiven-backed backend on mount. The database is the source of truth.
  useEffect(() => {
    const init = async () => {
      try {
        const remoteData = await backendService.getSiteData();
        setData(normalizeSiteData(remoteData as Partial<SiteData>));
      } catch (error) {
        console.error('Unable to load site data from the database:', error);
        setData(defaultData);
      } finally {
        setIsLoading(false);
      }
  ...
  const login = async (email: string, password: string, name?: string) => {
    try {
      ...
      const response = name
        ? await backendService.signup(name, email, password)
        : await backendService.login(email, password);

      const authenticatedUser = response.user as { name: string; email: string; role: 'admin' | 'user' };
      setCurrentUser(authenticatedUser);
      localStorage.setItem('ruthy_session_user', JSON.stringify(authenticatedUser));

      setData(prev => {
        const users = prev.users.some(u => u.email === authenticatedUser.email)
          ? prev.users.map(u => u.email === authenticatedUser.email ? { ...u, ...authenticatedUser } : u)
          : [...prev.users, { ...authenticatedUser, registeredAt: new Date().toISOString() }];
        return { ...prev, users };
      });
