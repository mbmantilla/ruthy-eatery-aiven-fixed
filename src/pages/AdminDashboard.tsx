      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-200 h-20 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center space-x-4">
             <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-full ${dbStatus.status === 'connected' ? 'bg-green-50' : 'bg-red-50'}`}>
                <div className={`w-2 h-2 rounded-full animate-pulse ${dbStatus.status === 'connected' ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className={`text-[10px] font-bold uppercase tracking-widest ${dbStatus.status === 'connected' ? 'text-green-700' : 'text-red-600'}`}>
                  {dbStatus.status === 'connected' ? 'Database Live' : 'Database Offline'}
                </span>
             </div>
             <button
               onClick={() => updateData(data)}
               className="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2 rounded-lg font-bold text-sm shadow-lg shadow-amber-600/20 flex items-center gap-2"
             >
